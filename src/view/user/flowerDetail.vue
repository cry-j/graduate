<!--
 * @Author: Jing
 * @Date: 2024-04-19 13:46:35
 * @LastEditTime: 2024-06-02 15:58:54
 * @Description: 花卉资料页
-->

<template>
    <p class="tittle">花卉资料</p>
    <el-divider style="margin-top: 10px" />
    <div class="myTable" style="padding-left: 30px; padding-right: 30px">
        <el-table
            :data="typeList"
            style="font-size: 20px"
            width="100%"
            max-height="450px"
            table-layout="auto"
            :row-style="{ height: '50px' }"
        >
            <el-table-column
                fixed
                label="序号"
                type="index"
                :index="1"
                width="80"
            />
            <el-table-column prop="typeName" label="类别名称" width="150" />
            <el-table-column prop="family" label="属" width="200" />
            <el-table-column
                prop="temperature"
                label="适宜温度"
                width="280"
                show-overflow-tooltip
            />
            <el-table-column
                prop="sunshine"
                label="日照"
                width="280"
                show-overflow-tooltip
            />

            <el-table-column
                prop="florescence"
                label="花期"
                width="280"
                show-overflow-tooltip
            />
            <el-table-column
                prop="origin"
                label="原产地"
                width="280"
                show-overflow-tooltip
            />
            <el-table-column fixed="right" label="操作" width="80">
                <template #default="scope">
                    <el-button
                        link
                        type="primary"
                        size="large"
                        @click="flowerDetail(scope.$index, scope.row)"
                    >
                        Detail
                    </el-button>
                </template>
            </el-table-column>
        </el-table>
        <el-dialog v-model="DialogVisible" :title="type.typeName" width="900">
            <el-divider style="margin-top: 10px" />
            <el-row :gutter="20">
                <el-col :span="14" :offset="0">
                    <ul>
                        <li>
                            <span>科属：</span>
                            <span>{{ type.family }}</span>
                        </li>
                        <li>
                            <span>原产地：</span><span>{{ type.origin }}</span>
                        </li>
                        <li>
                            <span>花果期：</span
                            ><span>{{ type.florescence }}</span>
                        </li>
                        <li>
                            <span>日照：</span><span>{{ type.sunshine }}</span>
                        </li>
                        <li>
                            <span>温度：</span
                            ><span>{{ type.temperature }}</span>
                        </li>
                        <li>
                            <span>土壤：</span><span>{{ type.soil }}</span>
                        </li>
                        <li>
                            <span>水分：</span><span>{{ type.water }}</span>
                        </li>
                    </ul>
                </el-col>
                <el-col :span="10" :offset="0"
                    ><el-image
                        :src="
                            'https://bkimg.cdn.bcebos.com/pic/' + type.flowerPic
                        "
                        fit="scale-down"
                        :min-scale="0.7"
                        :max-scale="2"
                        ><template #placeholder>
                            <div class="image-slot">
                                Loading<span class="dot">...</span>
                            </div>
                        </template></el-image
                    ></el-col
                >
            </el-row>
            <el-divider style="margin-bottom: 5px" />
            <p style="color: rgb(255, 86, 13); font-size: 25px">描述</p>
            <el-row :gutter="20">
                <el-col :span="23" :offset="0">
                    <p style="font-size: 18px">
                        &emsp;&emsp;{{ type.description }}
                    </p>
                </el-col>
            </el-row>

            <template #footer>
                <div class="dialog-footer">
                    <el-button type="primary" @click="DialogVisible = false">
                        确定
                    </el-button>
                </div>
            </template>
        </el-dialog>
    </div>
</template>

<script>
const baseUrl = "https://bkimg.cdn.bcebos.com/pic/"
export default {
    data() {
        return {
            typeList: [],
            type: {

            },
            DialogVisible: false,//控制详情展示页显示

        }
    },
    /**
     * @return {*}
     * @Date: 2024-06-02 15:58:41
     * @description: 加载初始信息
     */    
    mounted() {
        this.axios.get('/api/user/getAllTypes.do').then((result) => {
            let rs = result.data.dataList;
            this.typeList = rs;
        }).catch((err) => {
            alert(err.message)
        });
    },
    methods: {
        /**
         * @param {*} index
         * @param {*} row
         * @return {*}
         * @Date: 2024-06-02 16:00:11
         * @description: 详情展示
         */        
        flowerDetail(index, row) {
            this.type = row
            console.log(this.type)
            this.DialogVisible = true
        }
    },
}
</script>

<style scoped>
.tittle {
    font-size: 30px;
    margin-left: 20px;
    padding-top: 20px;
}
.myTable {
    /* width: 90%; */
}
:deep(.el-dialog__title) {
    font-size: 30px;
    margin-left: 20px;
}
ul li {
    font-size: 20px;
    line-height: 30px;
}
ul > li span:nth-child(1) {
    color: rgb(255, 134, 13);
}
</style>